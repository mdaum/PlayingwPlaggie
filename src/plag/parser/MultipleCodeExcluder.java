...
public class MultipleCodeExcluder
    implements CodeExcluder
{
    private ArrayList excluders;

    public MultipleCodeExcluder() {
	excluders = new ArrayList();
    }

    public MultipleCodeExcluder(ArrayList excluders) {
	this.excluders = excluders;
    }

    public void addCodeExcluder(CodeExcluder exc) {
	this.excluders.add(exc);
    }
    ...
    public void mark(TokenList tokensA,
		     ...
		     boolean[] marksB)
    {
	Iterator i = excluders.iterator();
	while (i.hasNext()) {
	    CodeExcluder exc = (CodeExcluder)i.next();
	    exc.mark(tokensA, tokensB,
		     marksA, marksB);
	}
    }
}
